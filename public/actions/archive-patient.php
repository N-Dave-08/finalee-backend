...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        ...
        try {
            ...
            $copy = $db->prepare('INSERT INTO archived_users (id, username, first_name, last_name, middle_name, date_of_birth, place_of_birth, gender, password, email, contact_num, role, created_at, reset_token, reset_token_expiry) SELECT id, username, first_name, last_name, middle_name, date_of_birth, place_of_birth, gender, password, email, contact_num, role, created_at, reset_token, reset_token_expiry FROM user WHERE id = ?');
